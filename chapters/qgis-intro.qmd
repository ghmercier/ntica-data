...
Android. Nous allons utiliser la version LTS disponible sur le
...
utilisateur ou sur une clé USB. Une version portable de QGIS est disponible sur
le blog [SIG & Territoires](https://www.sigterritoires.fr/index.php/version-portable-de-qgis-3-44-0/).
...
Si nous devions ajouter des couches exprimées dans un autre SCR, QGIS ferait la
conversion de manière transparente, à condition qu'il puisse déterminer sans
ambiguité le SCR de la couche fournie.
...
Pour mieux voir les entités vectorielles que l'on ajoutera par la suite, on 
peut déjà rendre le fond de plan transparent et le passer en niveau de gris.
...
Télécharger la [géodatabase](/resources/intro-qgis/ade_simplifiee.gpkg) préparée spécialement 
...
À chaque objet de la couche `communes` correspondent des données. On peut 
les consulter avec l'outil **Identifier des données** accessible par la 
barre d'outils. Pour les communes, un champ important est le `code_insee`. 
C'est lui qui identifie la commune de manière unique, on en aura besoin 
ultérieurement.
...
de toujours consulter les métadonnées d'une source pour s'assurer 
d'avoir bien tout compris. Dans le cas présent, on ira voir sur le 
...
- **Couche → Ouvrir la table d'attributs** ou <kbd>F6</kbd>.
...
- dans le menu déroulant, les modes de sélections sont possibles:
  rectangle de sélection, polygone, main levée, par rayon.
- des raccourcis-claviers utiles : <kbd>SHIFT</kbd> pour ajouter 
  des objets à la sélection courante, <kbd>CTRL</kbd> pour les 
  retrancher.

Sélectionner les trois communes de Champs-sur-Marne, Noisy-le-Grand et Émerainville.
...
105 829 habitants (au 1er janvier 2022).
</details>
...
via le bouton en forme de boulier dans la barre d'outils, également 
...
2 620 ha. L'écart est de 50 ha avec la superficie cadastrale, marge 2%.
</details>
...
Si vous avez fini le TD et que vous avez peur de vous ennuyez, 
voici quelques défis.

### Géoréférencement

Saurez-vous retrouver le plan d'aménagement partiel du secteur 4 de Marne-la-Vallée de mars 1987,
le géoréférencer et l'afficher dans QGIS ?

![](/resources/intro-qgis/16_secteur4.jpg)

Indices :

- Une 
  [page internet](https://www.inventerlegrandparis.fr/seminaire-igp/seminaire-igp-2019-2020/amenager-la-banlieue-parisienne-production-et-evolution-des-grands-equipements-structurants/le-val-deurope-a-marne-la-vallee-variations-sur-un-parc-a-theme/) 
  où trouver l'image source.
- Mettre la 
  [BD Parcellaire](https://cartes.gouv.fr/rechercher-une-donnee/dataset/IGNF_BD-PARCELLAIRE) en fond 
  pour trouver des points de contrôle.
- Le géoréférenceur est disponible via le menu  **Couches → Géoréférenceur**.
- Les points de contrôle que j'ai utilisés sont 
  [ici](/resources/intro-qgis/ill3-plan-masse-secteur-4-wing-chao-vs-jean-poulit-mars-1987.jpg.points).

### Géocodage d'adresses

Saurez-vous cartographier les bureaux de tabacs de la CA PVDM ?

![](/resources/intro-qgis/17_tabacs.jpg)

Indices :

- Furetez sur data.gouv.fr.
- Un géocodeur officiel est disponible via la 
  [Base Adresse Nationale](https://adresse.data.gouv.fr/)
- Analysez la qualité des données 

### Buffers / zones tampons

Saurez-vous cartographier les écoles de la CA PVDM qui sont situées à moins de 200 m d'un tabac ?

![](/resources/intro-qgis/18_tabac200m.jpg)

Indices :

- Le jeu de données des écoles est sur data.gouv.fr