...
Dans ce chapitre, nous allons apprendre à manipuler un type de fichier texte
...
Chargez les données dans une liste de dictionnaires 
que vous nommerez `communes`.

Affichez le nom et la population de la 10000^e^ ville de la liste.
...
Chargez les données dans une liste de dictionnaires que vous nommerez `depts`.
...
```{python}
# De façon naïve, avec une boucle traditionnelle
communes_avec_pb = []
for com in communes:
    if len(com) != 4:
...
# De façon pythonique, avec une compréhension de liste
...
On les élimine de la liste `communes`.
